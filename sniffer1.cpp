...
void packet_handler(u_char *user, const struct pcap_pkthdr *pkthdr, const u_char *packet) {
    ...
    // The ARP header starts 14 bytes after the beginning of the packet
    const u_char *arp_ptr = packet + 14; 

    // Byte 8-13 is the Sender MAC
    // Byte 18-23 is the Target MAC
    
    cout << BOLD << MAGENTA << "[ARP] " << RESET;
    
    cout << "Sender MAC: ";
    for(int i=0; i<6; i++) {
        printf("%02x%c", arp_ptr[8+i], (i==5) ? ' ' : ':');
    }

    cout << " | Target MAC: ";
    for(int i=0; i<6; i++) {
        printf("%02x%c", arp_ptr[18+i], (i==5) ? ' ' : ':');
    }
    cout << endl;
    return;
}
